...
<head>
    ...
    <title>区域設定あり(少数) + *CLAUSE_ZONE_Lv1</title>
    <style>
    ...
    <div class="container">
        <div class="header">
            <h1>区域設定あり(少数) + *CLAUSE_ZONE_Lv1</h1>
            <p>地区設定が「区域設定あり(少数)」かつ、コーディング「*CLAUSE_ZONE_Lv1」を含む段落</p>
        </div>
        <div class="controls">
...
def fetch_rows(db_path: Path):
    ...
    try:
        ...
        LEFT JOIN paragraph_codings pc ON pc.paragraph_id = p.id
        LEFT JOIN coding_types ct ON ct.id = pc.coding_type_id
        WHERE m.area_type = '区域設定あり(少数)'
        GROUP BY p.id
        HAVING SUM(CASE WHEN ct.code = '*CLAUSE_ZONE_Lv1' THEN 1 ELSE 0 END) > 0
...
if __name__ == '__main__':
    ...
    versioned_path = make_dated_versioned_path(out_dir, 'district_setting_minor_from_db_', '.html')

    # バージョン付きファイルを出力
    export_html(db, versioned_path)

    # 既存の固定名にも最新を上書き保存（後方互換）
    latest_path = out_dir / 'district_setting_minor_from_db.html'
