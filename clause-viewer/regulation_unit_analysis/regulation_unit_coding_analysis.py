...
def categorize_7level(row):
    """規制単位、area_type、regulation_typeを考慮して7段階に分類する
    
    Lv2(区域規制)を許可制優位と届出制優位で分割:
    - 1a_Lv2(区域・許可制優位)
    - 1b_Lv2(区域・届出制優位)
    """
...
def create_7level_chart(df, output_path):
    ...
    label_names = {
        '1a_Lv2(区域・許可制優位)': 'Lv2 (区域・許可制優位)',
        '1b_Lv2(区域・届出制優位)': 'Lv2 (区域・届出制優位)',
        '2_Lv1不同意(禁止)': 'Lv1 不同意 (禁止)',
        '3_Lv1許可or同意': 'Lv1 許可 or 同意',
        '5_抑制区域+届出': '抑制区域 + 届出・協議',
        '6_区域なし+許可制': '区域なし + 許可制',
        '7_届出・協議のみ': '届出・協議のみ'
    }
...
def create_7level_municipality_distribution_chart(df_7level, output_path):
    ...
    # タイトル拡大: 14 -> 24
    ax.set_title(f'7段階カテゴリ別 自治体数分布（総計: {total_munis}自治体）', fontsize=24, fontweight='bold')
    # 凡例拡大: 14 -> 18
...
def create_municipality_proportion_chart(df, output_path):
    ...
    for idx, code in enumerate(TARGET_CODINGS):
        ...
        ax.set_xticks(x)
        # X軸ラベル(カテゴリ名)拡大: 10 -> 14（7項目で幅が狭くなるため少し小さめ）
    ...
    # 全体タイトル拡大: 15 -> 28
    plt.suptitle('7段階規制カテゴリ別 - コーディング条文を持つ自治体の割合', 
...
def create_specific_variable_chart(df, output_path):
    ...
    plt.suptitle('7段階規制カテゴリ別 - 特定項目（騒音・資金・情報・同意）の保有割合', 
                 fontsize=28, fontweight='bold', y=0.95)
    plt.tight_layout(h_pad=5.0, w_pad=3.0, rect=[0, 0.03, 1, 0.95])
    plt.savefig(output_path, dpi=150, bbox_inches='tight')
    plt.close()
...
def main():
    ...
    try:
        ...
        # デザイン修正対象1: 自治体数分布グラフ
        create_7level_municipality_distribution_chart(df_7level, f"{OUTPUT_DIR}/municipality_distribution_7level.png")
